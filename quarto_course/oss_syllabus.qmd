...
header <- tribble(
  ...
  "Teaching Assistant", "Will Beason",
  "TA Office Hours Availability", "Thursdays and Fridays 9am-3pm by appointment",
  "TA Office Hours Link", "https://calendly.com/beason-utexas/30min",
  "TA Contact Email", "wab773@eid.utexas.edu"
)
library(knitr)
```
...
The late policy for papers/projects/presentation is a 10% reduction (minimum one point) for submissions up to 24 hours after the due time, but zero points after that.
If assignment answers are discussed in class, submissions will not be accepted after.
You can, of course, contact me if you have an emergency.
...
no_meetings <- tribble(
  ...
  #--------------------
   "2025-01-15", "2025-01-15", "No meeting: Instructor travel",
   "2025-01-20", "2025-01-20", "No meeting: MLK day",
 #  "2025-01-28", "2025-02-02", "No meeting: Instructor travel",
  
   # "2025-03-12", "2025-03-12", "", 
     "2025-03-17", "2025-03-22", "No meeting: Spring Break",
 #  "2024-04-08", "2024-04-08", "No meeting: Eclipse Day"
) %>%
  mutate(days_list = map2(period_start, period_end, expand_days)) %>%
  select(class_name, days_list) %>%
  unnest()

first_class_day = parse_datetime("2025-01-13")

last_class_day = parse_datetime("2025-04-28")

first_mon = parse_datetime("2025-01-13")

first_wed = parse_datetime("2025-01-15")
