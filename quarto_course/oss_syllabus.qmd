...
subtitle: "I320D and I320S: Open Source Software Development"
...
header <- tribble(
  ~`Course`, ~`I 320D and 320S: Open Source Software Development`,
  "Professor", "James Howison",
  "Meeting Time", "Mondays and Wednesdays 9:30 AM - 10:45 AM",
  "Location", "GDC 4.304",
  "Semester", "Spring 2026",
  "Unique No.", "28630",
  "Office Hours", "By appointment booked at http://james.howison.name/calendar/",
  "Contact Email", "jhowison@ischool.utexas.edu",
  "Teaching Assistant", "Ansh Disawal",
  "TA Office Hours Availability", "Wednesday and Fridays 9am-3pm by appointment",
  "TA Office Hours Link", "Book at: https://outlook.office.com/bookwithme/user/3b745980aaf140258b05fad914aa7e92@eid.utexas.edu/meetingtype/s-T9dOdAwEScIRKU_L9gEA2?anonymous&ismsaljsauthenabled&ep=mlink",
  "TA Contact Email", "ansh.disawal@utexas.edu"
)
library(knitr)
```
...
Jump to [Course Outline](#outline)
...
assessments <- tribble(
  ...
  #-----------------------
  "Class Participation (discussion and activites)", "15%", "Throughout course",
  "Weekly Homework challenges", "40%", "Weekly homework challenges throughout the semester",
  "Git Analogies Paper", "20%", "Week after Spring Break",
 # " UT Austin open source projects", "5%", "Two weeks after Spring Break",
  "Final Project and Interview", "25%", "Start of last class week.",
)
```
...
The course will have assignments based on both insight and skills topics we are learning (with some leaning towards skills), including use of the DataCamp course (see below).
...
These are open source projects funded by the US National Science Foundation during the last decade in a program called SI2. The data is collected from GitHub using the augur project from Linux Foundation. The files when unzipped are quite large, commits.csv has over 4.5 million rows. You may not be able to work with these files in Excel (for example) but they should work ok with tools like Tableau or Pandas/Matplotlib.
...
A 10 minute interview to discuss the final project, worth 40% of the Final Project grade. In this interview I will ask you to explain any part of your project or paper. For example I may ask how you came up with ideas, what particular lines of code do, or challenge parts of your argument. Just as it would be in a job interview, it will be unacceptable to say, "I don't know, GenAI did it" or "I don't know, I haven't thought about that".

### Late submission policy

The late policy for papers/projects/presentation is a 10% reduction (minimum one point) for submissions up to 24 hours after the due time, but zero points after that. That said, submissions will not be accepted after assignment answers have already been discussed in class. You can, of course, contact me if you have an emergency.
...
readings <- tribble(
  ...
  "Rewinding work @sec-rewinding and Managing Secrets @sec-history",
...
skills_topics <- tribble(
  ...
  "Paper Planes: Version Control @sec-planes-git", "git add, commit via paper planes and tables",  
"Git basic workflow @sec-git_basic_workflow", "git add, commit (locally)",
#  "Commits, blobs, and tree", "git diff continued HEAD and HEAD~1",
  "Rewinding work @sec-rewinding ", "revert, other undos, checking out old versions, rewriting history",
  "Branching @sec-branching ", "git checkout, merge.",  
  "Sharing and collaborating via github @sec-github-solo ", "git clone, remote, push",
  "Collaboration conflicts and workflows @sec-github-collaboration ", "github fork, pull request, pull upstream",
  "Splitting PRs @sec-cherrypick ", "merge, conflicts, mark resolved, cherrypick, collaboration workflows  ",
  "Rebase @sec-rebase", "git rebase",
  # "Git analogies paper workshop", "quarto and markdown",
 
#   "Documentation and Q&A", "github flavored markdown",
  "Tests @sec-tests", "pytest",
  "Continuous Integration @sec-ci", "Github Actions",
  # "Release management", "github releases, git ta g",
  "Creating and distributing packages @sec-packaging", "python packages, pypy pinning",
...
insight_topics <- tribble(
  ~insight_topic,
  #----------
  "Syllabus Review & Paper Planes and Innovation @sec-paper-planes-and-innovation",
  "What is open source? @sec-what-is-open-source",
  "Motivations and Asking questions people want to answer @sec-motivations ",
  "Licenses @sec-licenses",
  "Coordination @sec-coordination ",
  "Governance and decision making @sec-gov ",
  "Examining Real World Projects @sec-real-projects",
#  "Different kinds of open source projects",
  "Bias and lack of diversity @sec-diversity ",
#  "Iterative Development, Tests, and Continuous Integration @sec-agile ",
  "The stack and the stream @sec-stack-and-stream ",
  "Cybersecurity and open source @sec-cybersecurity ",
  "Open software work in science and research @sec-scisoft ",
...
no_meetings <- tribble(
  ...
  #--------------------
  "2026-01-19", "2026-01-19", "No meeting: MLK day",
  "2026-01-26", "2026-01-27", "No meeting: Ice storm",
  "2026-02-16", "2026-02-16", "No meeting: Instructor travel",
  "2026-03-16", "2026-03-21", "No meeting: Spring Break",
  "2026-03-25", "2026-03-25", "No meeting: Instructor travel"
) %>%
  mutate(days_list = map2(period_start, period_end, expand_days)) %>%
  select(class_name, days_list) %>%
  unnest()

first_class_day = parse_datetime("2026-01-12")

last_class_day = parse_datetime("2026-04-27")

first_mon = parse_datetime("2026-01-12")

first_wed = parse_datetime("2026-01-14")
...
#  left_join(presentation_topics, by = c("topic", "topic_order" = "rowname")) %>% 
  mutate(class_name = coalesce(class_name, insight_topic, skill_topic)) %>%   # mutate(class_name = coalesce(class_name, insight_topic, skill_topic, introduction_topic, presentation_topic)) %>% 
