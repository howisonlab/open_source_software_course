...
Remember, git swaps out all the **tracked** files when you do a `checkout`.  Experiment to find out what happens to **untracked** files:
  - Create a file called `my-untracked-file` run `git status` 
  - Run `git checkout <earlier-commit>` (where `<earlier-commit>` is a hash for your first commit)
