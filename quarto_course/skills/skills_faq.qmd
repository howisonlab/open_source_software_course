...
### Password pop-up repeats too often
...
Rstudio is generating that pop-up, but the frequency is controled by a gitconfig variable. The default without configuration is 15m so if we don't run a git command for 15m it expires the cache and pops up again. We can make it show up less with:
