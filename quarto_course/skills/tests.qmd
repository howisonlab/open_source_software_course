...
3. Add and commit, with a message indicating that tests should fail.
4. Shift the ValueError assertion into its own test
5. Implement a test that checks that a ValueError is raised if the input is not all digits. (Hint: you can use the `.isdigit()` method on a string)
6. Add and commit, with a message indicating that the test now documents why the tests are failing.
7. Revise the `fix_phone_num` function so that the tests now pass.  You can use the `replace` method to remove nondigit characters (or other techniques).
