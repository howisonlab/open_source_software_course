...
They help avoid the pain of your changes breaking other people's code; they are a clear marker of being a competant professional.
...
## Ensure Python, pytest, and quarto installed

We will be using Python, pytest and quarto today.  We added sections to the tech setup page at @sec-install, please visit and ensure these are installed.
...
```python
assert 1==1 # True, assertion passes
assert 1==2 # False, assertion fails
...
To run from the command line we need to move to Terminal. 
...
This brings us to another idea in testing: we should write the test first, see that it fails, then write the code.  This feels a little backwards, but the idea is that helps quality. 
...
----> 4     raise ValueError("Can only format numbers that are exactly 10 digits long")
      ...
      7   area_code = phone_num_to_fix[0:3] # 512 (first three digits)
...
In this exercise you will practice creating and commit failing tests for particular types of inputs and eventually you will alter the code so that those tests pass.  You will also practice communicating your status through commit messages so your teammates can understand your work.

Start with a fork of the [i320d-pytest-exercise](https://github.com/howisonlab/i320-pytest-exercise) repo and clone it to your workspace. If you edited the files you can remove the edits with `git checkout --`.

You can open and edit the my_code.py file in Rstudio or VSCode; you will run the tests at the command line using `pytest`.

1. Add an additional assertion to the `test_fix_phone_num` function that tests these inputs: `5554429876` and `3216543333`.
2. Add a test (meaning a whole separate function starting with `test_`, not just additional asserts) that specifies that the function should return an error, given these problematic inputs: `555-442-98761` and `(3213) 654 3333`. Give your test a communicative name.  These tests will fail at the momen; this is normal.
3. Add and commit, with a message indicating that you know that these tests fail currently and explaining your work plans.
4. Now implement a new test that checks that a ValueError is raised if these inputs are provided `334dfdee45` or `abcdefghij`). These are still 10 long, but they include non-digit characters. **Hint**: Python strings have a built-in method .isdigit() that returns True if all characters in the string are numeric digits — try it with `"abc123".isdigit()` vs `"5125558823".isdigit()`
...
The idea of writing tests first even thought they fail is called [Test Driven Development](https://en.wikipedia.org/wiki/Test-driven_development)
