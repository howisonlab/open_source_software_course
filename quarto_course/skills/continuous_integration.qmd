...
1. **Set up CI on your copy of a repo**
   - Get a copy of this repo [howisonlab/i320d-cicd-exercise](https://github.com/howisonlab/i320d-cicd-exercise) repository 
   ![Screenshot of Use Repository Option to create copy](continuous_integration_files/new-use-repo.png)
   - Clone the new copy to your laptop. (Note: this [creates a copy and not a fork](https://docs.github.com/en/repositories/creating-and-managing-repositories/creating-a-repository-from-a-template#creating-a-repository-from-a-template))
   - Working in Rstudio, create a file at `.github/workflows/pytest.yaml` with the full workflow contents from above. You will need to create the folders (first `.github`, then `workflows` inside that, then the `pytest.yaml` file)
   - Commit this to your `main` branch and push to your repository.

2. **Confirm CI is working**
   - Create a new branch and switch to it (e.g., `git checkout -b error_branch`)
   - Make a small edit to `README.md` (e.g., add "Testing CI"), save, add, commit, and push to GitHub.
   - Open a pull request **within your own copy** to merge your branch into your own `main`. Do not merge it yet.
   - Observe the tests running from the PR screen. They should pass — confirming that your CI setup is working.

3. **Introduce and fix a bug**
   - On the same branch, copy the content from `fix-phone-num-buggy.py` into `fix-phone-num.py`. This version has a bug which will cause tests to fail. Commit and push. (Note: you can also rename the files rather than copying).
   - Observe the tests running again from **the same PR** (because you pushed to the same branch). Locate the test failure in the Details view.
   - Back on your laptop and still on the same branch, fix the bug so the new tests pass. Save, Add, Commit and push. (You'll need to figure out the fix yourself.)
   - Observe the tests running once more. Your PR should now show "All checks have passed."
   - Accept your own PR (moving your bug fix from your `error_branch` to your main)
