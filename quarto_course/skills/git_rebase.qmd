...
::: {.callout-notice}
It's worth thinking about the power dynamics here.  

- How would this sort of request potentially affect new contributors?
- Who should pay the costs of integrating new contributions?
- What coordination impacts might de-synchronized branches have?
:::
...
::: {.callout-caution}
If you see a different UI than in the screenshot you might be using `vi` as the editor.  Quit using {{< kbd esc >}}-{{< kbd esc >}}`::q!` and then run `git config --global core.editor "nano"`
:::
...
(Since you are not the owners of the repo, you won't be able to do a force push here, but you could on your own fork).
...
Requiring contributors to use squash with their contributions is an example of a contribution policy. Many projects document these in a `CONTRIBUTING.md` file at the top level of their repo.  You can see examples:

- <https://github.com/sigstore/.github/blob/main/CONTRIBUTING.md>
- <https://github.com/squashimodo/react-table/blob/master/CONTRIBUTING.md>
- <https://contributing.md/example/>

Rebase and Merge workflows are both useful, but what trade offs are there?

Up to now, we’ve mostly talked about $$\text{git merge}$$ vs $$\text{git rebase}$$ in technical terms (shape of history, avoiding conflicts, etc.). For a socio‑technical view, it helps to compare them explicitly on *social* dimensions like cognitive load, power, and newcomer experience.

### **Technical + Social Comparison**

| Aspect | $$\text{Merge}$$ | $$\text{Rebase}$$ |
|-------|------------------|-------------------|
| History shape | Keeps explicit branching and merge commits; the timeline shows “what actually happened.” | Rewrites the branch so it looks like it was built directly on top of the current main branch; history appears linear. |
| Cognitive load for reviewers | Can be higher when there are many branches and merge commits; reviewers must mentally reconstruct the sequence. | Often lower when the contributor squashes and reorders commits; reviewers see a clean, linear story of the change. |
| Attribution & learning | Preserves granular steps (including “mistakes” and “fix typo” commits); useful for understanding how people actually worked and learned. | Squashing and rewriting can hide the intermediate problem‑solving path; easier to read but less revealing of the learning process. |
| Newcomer experience | History feels “honest” and less scary (no rewriting); tools like $$\text{git log}$$ show all the messy reality. | History rewriting can feel risky and intimidating; mistakes during rebase (especially with $$\text{git push --force}$$) can lead to anxiety and lost work. |
| Governance norms | Some projects allow merge commits freely, especially when they want to preserve a full record of integration events. | Many projects adopt a “no merge commits” or “please rebase before merge” norm to keep history linear and “professional‑looking.” |
| Who bears coordination cost | More of the coordination work is left to maintainers and reviewers, who must interpret a complex history. | More of the coordination work is pushed onto contributors, who must manage rebases and keep branches up‑to‑date. |
