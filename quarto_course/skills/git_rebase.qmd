...
```sh
cd ~
git clone git clone https://github.com/jameshowison/320d_rebase_example.git
cd 320d-rebase-example
git checkout pie_chart_branch
```
...
```sh
$ git viz
* 83f6236 (HEAD -> pie_chart_branch, origin/pie_chart_branch) pie chart 3
* 6780d4e pie chart 2
* 54b2ecb pie chart 1
...
We have two branches (main and pie_chart_branch). pie_chart_branch branched off from main at 52bd3fd, and since then more work has been added to main (the database related commits). We also have three separate commits for `pie_chart_branch`. 
...
Rebase does offer some additional opportunities though. The textual UI shows more options than just 'pick'.  One otion is 'squash' which takes changes from multiple commits and presents them in a single commit.  This can make a git history easier for teammates to understand.
...
```sh
Successfully rebased and updated refs/heads/pie_chart_branch.
repl@70e1b3d7-2edb-4b9d-affe-b74ce4e8cc24:~/workspace/320d_rebase_example$ git viz
* c367b8a (HEAD -> pie_chart_branch) pie chart 1
| * ed1f195 (origin/pie_chart_branch) pie chart 3
| * 0ade683 pie chart 2
| * 467a235 pie chart 1
|/  
| * 958ed94 (origin/main, origin/HEAD, main) database 2
| * ab98ea5 database 1
|/  
* 4e8da02 second edit
* e39f9e1 first edit
