...
Git stores full versions of our workspaces.  This means all the files and folders in our source code project. Git stores full copies of work as it was.  In our exercise we will use a folded piece of paper as a proxy for the workspace. Think of it like a tray on which all our work sits.  We will have lots of copies of these workspaces. We will call these a "commit".
...
We will make a copy of the work a few times:
1. Copy on clone
1. When we start working on a new version. We need a copy from our repository so we can work on it.  We will call this "copy on checkout"

2. When we offer our changes back to others.  We will call this "copy on pull".
...
3. Improver1 does a "checkout" on that commit. This means making a copy of the commit (including the name, but not the comment) and the paper plane, as a starting point for work.
...
6. Take a pink postit note and write on it "Please come get `VGDU` and pass it to the Originator. This is your _pull request_.

7. The Originator will then do _a pull_ by looking at `VGDU` in Improver1's repository, make a copy and check it into their own repository.
...
9. Improver2 will then do a _checkout_ which means making another complete copy of `VGDU` (which is the work as it was when Improver1 finished).

10. Improver2 can then do new work.  Fold the tips of each wing up.

11. Improver2 can then commit that work back into their repository.  Cross out the existing name (`VGDU`) and create a new name, add a comment about the work that you've done.
...
2. Improver1 should edit the plane by _unfolding_ the nose.  Then check that commit into their repo (crossing out the parent name, creating a new one and adding a comment.)

3. Improver2 should edit the plane by _double folding_ the nose. Then check that commit into their repo (crossing out the parent name, creating a new one and adding a comment.)
...
The conflict exists because the nose can't be both _double folded_ and _unfolded_.
